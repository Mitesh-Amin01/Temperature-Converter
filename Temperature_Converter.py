...
def convert(fromInput,fromUnitVar,toUnitVar,resultVar):
    fromU = fromUnitVar.get().lower()
    toU = toUnitVar.get()[0].lower()
    
    try:
        val = float(fromInput.get())
    except:
        resultVar.set("Error!")
...
convertMenu = ttk.OptionMenu(leftFrame,convertVar,"Farenheit","Celcuius","Kelvin",style="convertTo.TMenubutton")
...
rightFram = tk.Frame(root,bg=SECONDARY_COLOR,width=450,height=510)
rightFram.place(x=450,y=0)

resultVar = tk.StringVar(root)
resultVar.set("")

resultLabel = tk.Label(rightFram,textvariable= resultVar,bg=SECONDARY_COLOR,fg=PRIMARY_COLOR,font=getFont(55,True))
